...
rowLimit = 4
colLimit = 12
# Greedy factor: Lower greedy, Higher random - Will be in the range of 0.2 to 0.0(declining exp. func.)
exp_seq <- seq(0,9, by = 0.01)
epsilon <- function(x){
  return(0.2*exp(x))
}
# Learning Rate
...
for(i in 1:20){
  ...
  for(j in 1:1000){
    ...
    nextStateAction = epsilonGreedy(currentStateAction, epsilon(exp_seq[i]), Q)
    
    # One Episode
    repeat {
      nextnextStateAction = epsilonGreedy(nextStateAction, epsilon(exp_seq[i]), Q)
